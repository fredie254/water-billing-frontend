...
export const Zones = () => {
  ...
  const [activeZone, setActiveZone] = useState<string>(''); // filter routes by zone
  ...
  // ── Summary stats ──────────────────────────────────────────────────────────
  const totalConnections  = zones.reduce((s, z) => s + z.totalConnections, 0);
  const activeConnections = zones.reduce((s, z) => s + z.activeConnections, 0);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            KIRIWASCO service area — Kirinyaga County
          </p>
        </div>
        <div className="flex gap-2">
          {tab === 'routes' && (
            <button className="btn-secondary btn-sm"><Route className="w-4 h-4" /> Add Route</button>
          )}
          {tab === 'zones' && (
            <button className="btn-primary"><Plus className="w-4 h-4" /> New Zone</button>
          )}
        </div>
      </div>
          ...
          {!loadingZones && (
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
              {filteredZones.map(zone => {
                ...
                const utilPct    = zone.totalConnections > 0
                  ? Math.round((zone.activeConnections / zone.totalConnections) * 100)
                  : 0;
                return (
                  <div
                    key={zone.id}
                  ...
                  >
                    <div className="card-body space-y-4">
                      ...
                      <div className="flex items-start gap-3">
                        <div className={cn('w-10 h-10 rounded-xl flex items-center justify-center text-white font-bold text-sm flex-shrink-0', SUB_COUNTY_ICON_BG[zone.subCounty])}>
                          {zone.code}
                        </div>
                        <div className="flex-1 min-w-0">
                          ...
                          <span className={cn('inline-block mt-1 px-2 py-0.5 rounded-full text-[10px] font-medium border', SUB_COUNTY_COLOR[zone.subCounty])}>
                            {zone.subCounty}
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-3 gap-2 text-center">
                        <div className="bg-gray-50 rounded-lg py-2">
                          <p className="text-sm font-bold text-gray-900">{zone.totalConnections.toLocaleString()}</p>
                          <p className="text-[10px] text-gray-500">Connections</p>
                        </div>
                        <div className="bg-gray-50 rounded-lg py-2">
                          <p className="text-sm font-bold text-green-600">{zone.activeConnections.toLocaleString()}</p>
              ...
              {/* Add zone card */}
              <div className="border-2 border-dashed border-gray-200 rounded-xl p-5 flex flex-col items-center justify-center gap-2 text-gray-400 hover:border-primary-300 hover:text-primary-500 cursor-pointer transition-colors min-h-[180px]">
                          ...
                          <tr key={route.id} className="hover:bg-gray-50 transition-colors group cursor-pointer">
                            ...
                            <td className="px-4 py-3">
                              <div className="flex items-center gap-2">
                                <span className={cn('w-2 h-2 rounded-full flex-shrink-0', SUB_COUNTY_ICON_BG[zone?.subCounty ?? ''])} />
                                <span className="text-gray-700">{route.zoneName}</span>
                              </div>
                            </td>
                            <td className="px-4 py-3">
                              <span className={cn('inline-block px-2 py-0.5 rounded-full text-[10px] font-medium border', SUB_COUNTY_COLOR[zone?.subCounty ?? ''])}>
                                {zone?.subCounty ?? '—'}
                              </span>
                            </td>
                            <td className="px-4 py-3 text-right">
                              <span className="font-semibold text-gray-900">{route.connectionCount.toLocaleString()}</span>
                            </td>
                            <td className="px-4 py-3">
                              {route.readerName ? (
                                <div className="flex items-center gap-2">
